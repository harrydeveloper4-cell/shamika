<x-admin-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="font-bold text-xl text-slate-100 leading-tight">{{ __('Edit Config') }}</h2>
            <a href="{{ route('admin.config.index') }}" class="px-3.5 py-1.5 border border-slate-100 text-slate-600 hover:bg-slate-50 text-xs font-semibold rounded-lg transition">← Back</a>
        </div>
    </x-slot>

    <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6 max-w-2xl mx-auto">
        <form action="{{ route('admin.config.update', $config->id) }}" method="POST" enctype="multipart/form-data" class="space-y-4">
            @csrf
            @method('PUT')
            <div>
                <label for="key" class="block text-xs font-bold uppercase tracking-wider text-slate-500 mb-1">Key</label>
                <input type="text" name="key" id="key" value="{{ old('key', $config->key) }}" required class="w-full text-sm rounded-lg border-slate-200 focus:border-indigo-500 focus:ring-indigo-500">
            ...
            <div>
                ...
                <select name="type" id="type_select" onchange="toggleValueInput(this.value)" class="w-full text-sm rounded-lg border-slate-200 focus:border-indigo-500 focus:ring-indigo-500">
                    <option value="text" {{ old('type', $config->type) == 'text' ? 'selected' : '' }}>Text</option>
                    <option value="image" {{ old('type', $config->type) == 'image' ? 'selected' : '' }}>Image</option>
                </select>
            ...
            <div id="text_input_container">
                ...
                <textarea name="value" id="value_text" rows="4" class="w-full text-sm rounded-lg border-slate-200 focus:border-indigo-500 focus:ring-indigo-500">{{ old('value', $config->value) }}</textarea>
                @error('value') <span class="text-xs text-rose-500 mt-1 block">{{ $message }}</span> @enderror
            </div>

            <div id="file_input_container" class="hidden space-y-2">
                ...
                @if($config->type == 'image' && $config->value)
                    <div class="mb-2">
                        <img src="{{ asset($config->value) }}" alt="Current Image" class="w-20 h-20 object-cover rounded-lg border border-slate-200">
                    </div>
                @endif
            ...
            <div class="pt-2 text-right">
                <button type="submit" class="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-semibold rounded-lg shadow-sm transition">Update Config</button>
            </div>
        </form>
    </div>
